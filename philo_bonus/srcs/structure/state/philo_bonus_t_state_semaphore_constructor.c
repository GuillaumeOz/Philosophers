...
/*   Updated: 2021/09/08 18:33:27 by gozsertt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	create_write_semaphore(t_state *state)
{
	...
	state->write_semaphore_three = sem_open("write_semaphore_three",
			O_CREAT, 0777, 0);
	if (state->write_semaphore_three == SEM_FAILED)
	{
		error_msg("Error: create_state_semaphore\
			malloc semaphore_three failed");
		state->error_state = true;
		return ;
	}
}
